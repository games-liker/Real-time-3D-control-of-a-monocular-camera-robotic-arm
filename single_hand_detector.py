...
class SingleHandDetector:
    ...
    def detect(self, rgb, return_wrist_world_pos=False):
        """
        检测手部
        
        Args:
            rgb: RGB图像
            return_wrist_world_pos: 是否返回原始手腕世界坐标（用于机械臂控制）
            
        Returns:
            如果 return_wrist_world_pos=False: (num_box, joint_pos, keypoint_2d, mediapipe_wrist_rot)
            如果 return_wrist_world_pos=True: (num_box, joint_pos, keypoint_2d, mediapipe_wrist_rot, wrist_world_pos)
        """
        results = self.hand_detector.process(rgb)
        
        if not results.multi_hand_landmarks:
            if return_wrist_world_pos:
        ...
        # 保存原始手腕位置（世界坐标）
        wrist_world_pos = keypoint_3d_array[0:1, :].copy()
        # 归一化到相对于手腕的坐标
        ...
        if return_wrist_world_pos:
            return num_box, joint_pos, keypoint_2d, mediapipe_wrist_rot, wrist_world_pos
